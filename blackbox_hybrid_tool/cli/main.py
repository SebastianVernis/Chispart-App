...
Ejemplos de uso:
  ...
  %(prog)s switch-model blackboxai/openai/o1
        ...
        parser.add_argument(
            "--debug",
            action="store_true",
            help="Imprime payloads y respuestas de la API",
        )
        ...
        shell_parser = subparsers.add_parser(
            "shell", help="Inicia una shell interactiva del proyecto con tema CHISPART"
        )
        shell_parser.add_argument(
            "--no-clear",
            action="store_true",
            help="No limpiar pantalla antes de iniciar",
        )

        # Comando para generar tests
        generate_parser = subparsers.add_parser(
            "generate-tests", help="Genera tests automáticamente para un archivo"
        )
        generate_parser.add_argument("file", help="Archivo fuente para generar tests")
        ...
        coverage_parser = subparsers.add_parser(
            "analyze-coverage", help="Analiza cobertura de código"
        )
        coverage_parser.add_argument(
            "path", help="Ruta al directorio de tests o archivo"
        )
        coverage_parser.add_argument(
            "-f",
            "--format",
        ...
        ai_parser = subparsers.add_parser("ai-query", help="Realiza consultas a la IA")
        ai_parser.add_argument("query", help="Consulta para la IA")
        ai_parser.add_argument("-m", "--model", help="Modelo AI a usar (opcional)")
        ...
        aidx.add_argument(
            "instruction", help="Instrucción de desarrollo (qué cambiar/agregar)"
        )
        aidx.add_argument(
            "-s",
            "--strategy",
            choices=["auto", "fast", "reasoning", "code"],
            default="auto",
            help="Estrategia/modelo sugerido",
        )
        aidx.add_argument(
            "-m", "--model", help="Identificador de modelo a forzar (opcional)"
        )
        aidx.add_argument(
            "--allow-web",
            action="store_true",
            help="Permitir búsqueda web y uso en contexto",
        )
        aidx.add_argument(
            "-e", "--engine", choices=["serpapi", "tavily"], help="Motor de búsqueda"
        )
        aidx.add_argument(
            "--apply",
            action="store_true",
            help="Aplicar automáticamente el parche si tests pasan",
        )
        aidx.add_argument(
            "--out-dir",
            default="patches",
            help="Directorio donde guardar el .patch propuesto",
        )
        ...
        repl_parser = subparsers.add_parser(
            "repl", help="Inicia una sesión de chat interactiva con contexto"
        )
        repl_parser.add_argument(
            "-m", "--model", help="Identificador de Blackbox a usar durante la sesión"
        )
        repl_parser.add_argument(
            "-s",
            "--session",
        ...
        wf_parser = subparsers.add_parser(
            "write-file", help="Crea o escribe un archivo de texto"
        )
        ...
        group_src.add_argument(
            "--stdin", action="store_true", help="Leer contenido desde STDIN"
        )
        group_src.add_argument(
            "-e",
            "--editor",
            action="store_true",
            help="Abrir editor ($EDITOR, nano o vi) para escribir el contenido",
        )
        wf_parser.add_argument(
            "--overwrite",
            action="store_true",
            help="Permitir sobrescribir si el archivo ya existe",
        )
        ...
        srcgrp.add_argument(
            "-f", "--file", dest="patch_file", help="Archivo de parche (.patch/.diff)"
        )
        srcgrp.add_argument(
            "--stdin", action="store_true", help="Leer parche desde STDIN"
        )
        ap_parser.add_argument(
            "--root",
            default=".",
            help="Directorio raíz sobre el que aplicar (por defecto: .)",
        )
        ap_parser.add_argument(
            "--dry-run", action="store_true", help="Simula sin escribir cambios"
        )
        ...
        gsrc.add_argument(
            "-f", "--file", dest="gist_file", help="Archivo a subir como Gist"
        )
        gsrc.add_argument(
            "--stdin", action="store_true", help="Leer contenido desde STDIN"
        )
        gh_gist.add_argument(
            "-n", "--name", default="snippet.txt", help="Nombre del archivo en el Gist"
        )
        gh_gist.add_argument(
            "-d", "--description", default="", help="Descripción del Gist"
        )
        gh_gist.add_argument(
            "--public",
            action="store_true",
            help="Gist público (por defecto es secreto)",
        )

        # Comandos de auto-repo
        ss = subparsers.add_parser(
            "self-snapshot",
            help="Genera y embebe un snapshot comprimido del repo actual",
        )
        se = subparsers.add_parser(
            "self-extract", help="Extrae el snapshot embebido a un directorio destino"
        )
        se.add_argument(
            "-o",
            "--out",
            default=".self_extract",
            help="Directorio de salida (por defecto ./.self_extract)",
        )
        sa = subparsers.add_parser(
            "self-analyze", help="Analiza dependencias y estructura (actual o snapshot)"
        )
        sa.add_argument(
            "--from",
            dest="source",
            choices=["current", "embedded"],
            default="current",
            help="Fuente del análisis",
        )
        st = subparsers.add_parser("self-test", help="Ejecuta tests en el árbol actual")
        sup = subparsers.add_parser(
            "self-apply-patch",
            help="Aplica parche en copia, corre tests y si pasan, sustituye",
        )
        ...
        sup.add_argument(
            "--use-embedded",
            action="store_true",
            help="Aplicar sobre snapshot embebido en lugar del árbol actual",
        )

        # Web: búsqueda y fetch
        ws = subparsers.add_parser(
            "web-search", help="Busca en la web (requiere SERPAPI_KEY o TAVILY_API_KEY)"
        )
        ws.add_argument("-q", "--query", required=True, help="Consulta de búsqueda")
        ws.add_argument(
            "-e",
            "--engine",
            choices=["serpapi", "tavily"],
            help="Motor de búsqueda a usar",
        )
        ws.add_argument(
            "-n", "--num", type=int, default=5, help="Número de resultados (default 5)"
        )
        wf = subparsers.add_parser(
            "web-fetch", help="Descarga una URL y la convierte a texto"
        )
        ...
        switch_parser = subparsers.add_parser(
            "switch-model", help="Cambia el modelo AI por defecto"
        )
        ...
        list_parser = subparsers.add_parser(
            "list-models", help="Lista los modelos AI disponibles"
        )

        # Comando para configuración
        config_parser = subparsers.add_parser(
            "config", help="Muestra configuración actual"
        )

        # Herramientas SSH / despliegue
        ssh_exec = subparsers.add_parser(
            "ssh-exec", help="Ejecuta un comando remoto via SSH"
        )
        ...
        ssh_sync = subparsers.add_parser(
            "ssh-sync", help="Sincroniza/copias archivos al remoto via SCP"
        )
        ...
        deploy = subparsers.add_parser(
            "deploy-remote", help="Despliegue remoto (Docker/Compose o sin Docker)"
        )
        ...
        deploy.add_argument(
            "--dir", required=True, help="Directorio del proyecto en el servidor"
        )
    ...
    def run_ai_query(self, args):
        ...
        try:
            ...
            response = self.ai_orchestrator.generate_response(
                args.query, model_type=args.model, debug=getattr(args, "debug", False)
            )
    ...
    def run_ai_dev(self, args):
        try:
            ...
            if (
                args.allow_web
                and (args.engine or os.getenv("WEB_SEARCH_ENGINE"))
                and (os.getenv("SERPAPI_KEY") or os.getenv("TAVILY_API_KEY"))
            ):
                try:
    ...
    def run_repl(self, args):
        ...
        if not current_model:
            try:
                current_model = (
                    self.ai_orchestrator.models_config.get("models", {})
                    .get("blackbox", {})
                    .get("model")
                )
            except Exception:
        ...
        transcript_path = (
            Path(args.transcript).expanduser() if args.transcript else None
        )
        ...
        print(
            "🛠️  Herramientas disponibles para el asistente: write-file, web-search, web-fetch, self-apply-patch"
        )
        print(
            'ℹ️  Para invocar herramientas, el asistente emitirá JSON: {"tool":"<name>", "args":{...}}'
        )
        if current_model:
        ...
        def exec_tool_call(tool_obj: dict):
            ...
            try:
                ...
                if name == "web-search":
                    ...
                    res = ws.search(
                        args_.get("query", ""), num_results=int(args_.get("num", 5))
                    )
                    return {
                        "status": "ok",
                ...
                if name == "web-fetch":
                    ...
                    out = {
                        ...
                        if k in ("url", "status", "content_type", "text_stripped")
                    }
                ...
                if name == "self-apply-patch":
                    ...
                    with tempfile.NamedTemporaryFile(
                        "w", delete=False, encoding="utf-8", suffix=".patch"
                    ) as tf:
                        tf.write(patch_text)
                        tf.flush()
                        tmpfile = tf.name
                ...
                if cmd in ("exit", "quit"):  # salir
                    ...
                    try:
                        ...
                        history.append(
                            {
                                "role": "system",
                                "content": f"TOOL_RESULT {tname}: {json_dumps(result)}",
                            }
                        )
                    except Exception as e:
                    ...
                    print(
                        "Comandos: /model <id>, /reset, /save, /session <nombre>, /transcript <ruta>, /tools [<name> [args_json]], /exit, /help"
                    )
                    continue
                else:
            ...
            while True:
                ...
                if tool_call and tool_steps < 5:
                    ...
                    history.append(
                        {
                            "role": "system",
                            "content": f"TOOL_RESULT {tool_call.get('tool')}: {json_dumps(result)}",
                        }
                    )
                    tool_steps += 1
                    continue
    ...
    def run_self_snapshot(self, args):
        try:
            ...
            if changed:
                print(
                    f"✅ Snapshot embebido actualizado (files={meta.get('file_count')}, hash={str(meta.get('sha256'))[:8]}...)"
                )
            else:
    ...
    def run_self_extract(self, args):
        try:
            ...
            print(
                f"✅ Extraído snapshot en: {info['path']} (files={info['meta'].get('file_count')})"
            )
            return 0
        except Exception as e:
    ...
    def run_self_apply_patch(self, args):
        try:
            ...
            code = subprocess.call(
                [sys.executable, "-m", "pytest", "-q"], cwd=str(workdir)
            )
            if code != 0:
                print(
                    f"❌ Pruebas fallaron en copia (exit={code}). No se aplican cambios."
                )
                return code
    ...
    def run_write_file(self, args):
        ...
        try:
            ...
            if dest.exists() and not args.overwrite:
                print(
                    f"❌ El archivo ya existe: {dest} (usa --overwrite para sobrescribir)"
                )
                return 1
            ...
            if args.content is not None:
                ...
                print(
                    "❌ Debes proporcionar contenido con --content, --stdin o --editor"
                )
                return 1
    ...
    def run_gh_status(self, args):
        try:
            ...
            print(
                f"Usuario: {me.get('login')} | ID: {me.get('id')} | Nombre: {me.get('name')}"
            )
            return 0
        except Exception as e:
    ...
    def run_gh_create_gist(self, args):
        try:
            ...
            gh = GitHubClient()
            result = gh.create_gist(
                {args.name: content}, description=args.description, public=args.public
            )
            print("✅ Gist creado:")
    ...
    def run_switch_model(self, args):
        ...
        try:
            ...
            if "/" in args.model:
                ...
                self.ai_orchestrator.models_config.setdefault("models", {}).setdefault(
                    "blackbox", {}
                )["model"] = args.model
                ...
                if args.model != "blackbox":
                    raise ValueError(
                        "Modelo inválido. Use 'blackbox' o un identificador de Blackbox con '/'."
                    )
    ...
    def run_list_models(self, args):
        ...
        for model_name, config in models_config.get("models", {}).items():
            status = (
                "✅ Habilitado" if config.get("enabled", False) else "❌ Deshabilitado"
            )
    ...
    def run_config(self, args):
        ...
        try:
            ...
            for model_name, config in models_config.get("models", {}).items():
                ...
                print(
                    f"    API Key: {'Configurada' if config.get('api_key') else 'No configurada'}"
                )
    ...
    def run_ssh_exec(self, args):
        try:
            return run_ssh_command(
                args.host, args.cmd, user=args.user, key_path=args.key, port=args.port
            )
        except Exception as e:
            print(f"❌ Error SSH: {e}")
            return 1
    ...
    def run_shell(self, args):
        ...
        if omp and theme_path.exists():
            env[
                "PROMPT_COMMAND"
            ] = f'PS1="$({omp} print primary --config {theme_path} --shell bash)"'
        else:
