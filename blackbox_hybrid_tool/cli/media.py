...
def download_media(url, model_name, extension_hint=None):
    ...
    try:
        ...
        if not ext:
            ...
            if "video/mp4" in content_type or extension_hint == ".mp4":
                ext = ".mp4"
            elif "image/webp" in content_type or extension_hint == ".webp":
                ext = ".webp"
            elif "image/jpeg" in content_type or extension_hint == ".jpg":
                ext = ".jpg"
            elif "image/png" in content_type or extension_hint == ".png":
                ext = ".png"
            else:
                ext = ".out"
        ...
        print(
            f"ℹ️ En la interfaz web, este {media_type.lower()} se mostrará automáticamente embebido en la conversación."
        )
...
def run_image_batch(orchestrator: AIOrchestrator, args):
    ...
    if profile:
        ...
        use_profile = (
            input("¿Usar este perfil? (s/n, 'n' para crear uno nuevo): ")
            .strip()
            .lower()
        )
        if use_profile != "s":
    ...
    while True:
        try:
            num_images = int(
                input("¿Cuántas imágenes quieres crear? (ej: 10): ").strip()
            )
            if num_images > 0:
                break
            else:
    ...
    use_multiprompt = (
        input(
            "\n¿Quieres usar generación multiprompt para crear imágenes relacionadas? (s/n): "
        )
        .strip()
        .lower()
        == "s"
    ...
    if use_multiprompt:
        ...
        try:
            ...
            prompt_segments = create_multiprompt_sequence(
                final_prompt, media_type="Image"
            )
            ...
            if model_limit > 1 and len(prompt_segments) > 1:
                        ...
                        try:
                            ...
                            if image_url and image_url.startswith("http"):
                                ...
                                downloaded_path = download_media(
                                    image_url, selected_model, extension_hint=".png"
                                )

                                if downloaded_path and profile.get("logo_path"):
                                    output_with_logo = (
                                        f"final_{os.path.basename(downloaded_path)}"
                                    )
                                    overlay_logo(
                                        downloaded_path,
                                        profile["logo_path"],
                                    ...
                                    print(
                                        f"🖼️ Imagen con logo guardada como: {output_with_logo}"
                                    )
                            else:
                                print(
                                    f"Respuesta inesperada (no es una URL): {image_url}"
                                )

                        except Exception as e:
                            print(
                                f"❌ Error al generar la imagen del segmento {prompt_index+1}: {e}"
                            )
            else:
                ...
                for i, segment_prompt in enumerate(prompt_segments):
                    ...
                    try:
                        image_url = orchestrator.generate_response(
                            segment_prompt, model_type=selected_model, max_tokens=1024
                        )
            ...
            try:
                ...
                if image_url and image_url.startswith("http"):
                    ...
                    if downloaded_path and profile.get("logo_path"):
                        ...
                        overlay_logo(
                            downloaded_path, profile["logo_path"], output_with_logo
                        )
...
def run_media_command(orchestrator: AIOrchestrator):
    ...
    parser = argparse.ArgumentParser(
        description="Herramienta de creación multimedia interactiva."
    )
    ...
    img_parser = subparsers.add_parser(
        "image-batch", help="Crear múltiples imágenes con un perfil de marca."
    )

    # Subcomando para perfiles
    prof_parser = subparsers.add_parser("profile", help="Gestionar perfiles de marca.")
    prof_sub = prof_parser.add_subparsers(
        dest="profile_subcommand", help="Acciones de perfil"
    )
    ...
    activate_parser = prof_sub.add_parser(
        "activate", help="Establecer un perfil como activo."
    )
    ...
    show_parser.add_argument(
        "--name", help="Nombre del perfil a mostrar (por defecto, el activo)."
    )
