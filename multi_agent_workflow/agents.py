...
class ResearcherAgent(BaseAgent):
    def __init__(self, api_key: str):
        ...
        self.tools = [
            search_web_tool_definition()
        ]  # Definición de la herramienta de búsqueda web
        self.model = (
            "blackboxai/openai/gpt-4o-mini"  # Modelo para razonamiento del investigador
        )
    ...
    async def research(self, query: str) -> str:
        ...
        messages = [
            {
                "role": "system",
                "content": "Eres un agente investigador experto. Tu tarea es usar las herramientas disponibles para encontrar información relevante.",
            },
            {"role": "user", "content": f"Necesito información sobre: {query}"},
        ]

        # Primer intento: dejar que el LLM decida si usar la herramienta
        response = await self._call_blackbox_chat(
        ...
        if (
            ...
            for tool_call in tool_calls:
                if tool_call["function"]["name"] == "search_web":
                    ...
                    print(
                        f"Investigador: Ejecutando herramienta search_web con query: {search_terms}"
                    )
                    ...
                    tool_results.append(
                        {"tool_call_id": tool_call["id"], "output": simulated_result}
                    )
            ...
            final_response = await self._call_blackbox_chat(
                messages, self.model, tools=self.tools
            )
...
class ContentGeneratorAgent(BaseAgent):
    def __init__(self, api_key: str):
        super().__init__(api_key)
        self.model = (
            "blackboxai/openai/gpt-4o-mini"  # Modelo para generación de contenido
        )
    ...
    async def generate_report(self, research_data: str) -> str:
        ...
        messages = [
            {
                "role": "system",
                "content": "Eres un agente generador de contenido experto. Tu tarea es redactar un reporte conciso y bien estructurado basado en los datos proporcionados.",
            },
            {
                "role": "user",
                "content": f"Redacta un reporte basado en los siguientes datos: {research_data}",
            },
        ]
    ...
    async def generate_image(self, prompt: str) -> str:
        ...
        image_model = (
            "blackboxai/black-forest-labs/flux-pro"  # Ejemplo de modelo de imagen
        )
        ...
        if "choices" in response and "content" in response["choices"][0]["message"]:
            return response["choices"][0]["message"][
                "content"
            ]  # Debería ser la URL de la imagen
        else:
