...
from main import (
    create_multiprompt_sequence,
    update_media_response_multi,
    IMAGE_MODEL_LIMITS,
)
...
class TestImageMultiprompt(unittest.TestCase):
    ...
    def test_create_multiprompt_sequence_for_images_success(self, mock_orchestrator):
        ...
        original_prompt = (
            "Un águila volando sobre las montañas y luego posada en un árbol"
        )
        prompt_sequence = create_multiprompt_sequence(
            original_prompt, media_type="Image"
        )
    ...
    @patch("main.orchestrator")
    def test_create_multiprompt_sequence_for_images_empty_response(
