...
def test_webfetcher_fetch_success(monkeypatch):
    ...
    monkeypatch.setattr(
        "blackbox_hybrid_tool.utils.web.requests.get", lambda url, timeout=15: R()
    )
...
def test_github_client_headers_and_calls(monkeypatch):
    ...
    # get_user
    with patch(
        "blackbox_hybrid_tool.utils.github_client.requests.get", return_value=R()
    ):
        assert client.get_user().get("ok") is True
    # create_gist
    with patch(
        "blackbox_hybrid_tool.utils.github_client.requests.post", return_value=R()
    ):
