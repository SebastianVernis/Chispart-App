...
def test_blackbox_client_debug_and_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "blackbox_hybrid_tool.core.ai_client.requests.post", lambda *a, **k: R()
    )
        ...
        json.dumps(
            {
                "default_model": "auto",
                "models": {
                    "blackbox": {"api_key": "k", "model": "blackbox", "enabled": True}
                },
            }
        ),
        encoding="utf-8",
        ...
        json.dumps(
            {
                "default_model": "auto",
                "models": {
                    "blackbox": {"api_key": "", "model": "blackbox", "enabled": True}
                },
            }
        ),
        encoding="utf-8",
...
def test_import_csv_exception_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "blackbox_hybrid_tool.core.ai_client.csv.DictReader", lambda f: BadReader()
    )
