...
class TestImageModelLimits(unittest.TestCase):
    ...
    def test_specific_model_limits(self):
        ...
        self.assertEqual(
            IMAGE_MODEL_LIMITS.get("blackboxai/black-forest-labs/flux-1.1-pro-ultra"), 1
        )

        # Modelos con límite de 4
        self.assertEqual(
            IMAGE_MODEL_LIMITS.get("blackboxai/black-forest-labs/flux-schnell"), 4
        )
        self.assertEqual(
            IMAGE_MODEL_LIMITS.get("blackboxai/bytedance/hyper-flux-8step"), 4
        )
        self.assertEqual(
            IMAGE_MODEL_LIMITS.get("blackboxai/stability-ai/stable-diffusion"), 4
        )

        # Modelos con límite de 10
        self.assertEqual(
            IMAGE_MODEL_LIMITS.get("blackboxai/prompthero/openjourney"), 10
        )
