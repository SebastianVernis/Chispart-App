#!/bin/bash
# Chispart-App deployment script for Render CLI

set -e

PROJECT_NAME="Chispart-App"
SERVICE_NAME="chispart-ai"
BRANCH="master"
...
if render services list --output json | jq -e ".[] | select(.name==\"$SERVICE_NAME\")" > /dev/null 2>&1; then
    ...
    sleep 3
    render logs "$SERVICE_NAME" --tail 50
    ...
    echo "    --plan free"
    echo ""
    echo "Don't forget to set BLACKBOX_API_KEY:"
    echo "  render env set BLACKBOX_API_KEY='your_key' --service $SERVICE_NAME"
    echo ""
